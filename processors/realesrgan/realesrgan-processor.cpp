  ...
  // initialize preprocess and postprocess pipeline
  {
    ...
    if (tta_mode) {
      if (net->opt.use_fp16_storage && net->opt.use_int8_storage)
        realesrgan_preproc->create(realesrgan_preproc_tta_int8s_spv_data, sizeof(realesrgan_preproc_tta_int8s_spv_data),
                                   specializations);
      else if (net->opt.use_fp16_storage)
        realesrgan_preproc->create(realesrgan_preproc_tta_fp16s_spv_data, sizeof(realesrgan_preproc_tta_fp16s_spv_data),
                                   specializations);
      else
      ...
      if (net->opt.use_fp16_storage && net->opt.use_int8_storage)
        realesrgan_postproc->create(realesrgan_postproc_int8s_spv_data, sizeof(realesrgan_postproc_int8s_spv_data),
                                    specializations);
      else if (net->opt.use_fp16_storage)
        realesrgan_postproc->create(realesrgan_postproc_fp16s_spv_data, sizeof(realesrgan_postproc_fp16s_spv_data),
                                    specializations);
      else
...
liret RealesrganProcessor::process_image(const ImageInfo &inimage, ImageInfo &outimage,
  ...
  // calculate number of image chunks. (total = xtiles * ytiles)
  ...
  for (int yi = 0; yi < ytiles; yi++) {
    ...
    if (opt.use_fp16_storage && opt.use_int8_storage) {
      ...
      if (channels == 3) {
        // TODO: create platform independent pixel format selection
#if _WIN32
        in = ncnn::Mat::from_pixels(pixeldata + in_tile_y0 * w * channels, ncnn::Mat::PIXEL_BGR2RGB, w,
      ...
      if (channels == 4) {
#if _WIN32
        in = ncnn::Mat::from_pixels(pixeldata + in_tile_y0 * w * channels, ncnn::Mat::PIXEL_BGRA2RGBA, w,
                                    (in_tile_y1 - in_tile_y0));
#else
    ...
    for (int xi = 0; xi < xtiles; xi++) {
      ...
      // TODO: link to log system
      procb((float)(yi * xtiles + (xi + 1)) / (ytiles * xtiles));
    }
    // download
    {
      ncnn::Mat out;
