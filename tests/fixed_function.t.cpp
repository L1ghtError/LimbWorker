...
template <typename T> T test_free_func_template(T p) { return p; }
...
template <typename T> struct Foo {
  template <typename U> U bar(U p) { return p + payload; }

  T payload;
};

template <typename T> void print_overhead() {
  using func_type = limb::tp::FixedFunction<void(), sizeof(T)>;
  int t_s = sizeof(T);
...
TEST(FixedFunction, allocDealloc) {
...

  {
    cnt c1;
    c1.payload = "xyz";
    limb::tp::FixedFunction<std::string()> f1(c1);
    ASSERT_EQ(std::string("xyz"), f1());

    limb::tp::FixedFunction<std::string()> f2;
    f2 = std::move(f1);
    ASSERT_EQ(std::string("xyz"), f2());

    limb::tp::FixedFunction<std::string()> f3(std::move(f2));
    ASSERT_EQ(std::string("xyz"), f3());

    limb::tp::FixedFunction<std::string()> f4(str_fun);
    ...
    cnt c2;
    c2.payload = "qwe";
    f4 = std::move(limb::tp::FixedFunction<std::string()>(c2));
...
TEST(FixedFunction, freeFunc) {
  limb::tp::FixedFunction<int(int)> f(test_free_func);
  auto f1 = std::move(f);
  ASSERT_EQ(3, f1(3));
};

TEST(FixedFunction, freeFuncTemplate) {
  limb::tp::FixedFunction<std::string(std::string)> f(test_free_func_template<std::string>);
  auto f1 = std::move(f);
  ASSERT_EQ(std::string("abc"), f1("abc"));
}

TEST(FixedFunction, voidFunc) {
  limb::tp::FixedFunction<void(int &, int)> f(test_void);
  auto f1 = std::move(f);
  int p = 0;
  f1(p, 42);
  ASSERT_EQ(42, p);
...
TEST(FixedFunction, classMethodVoid) {
  ...
  A a;
  int i = 0;
  limb::tp::FixedFunction<void(int &)> f(std::bind(&A::c, &a, _1));
  auto f1 = std::move(f);
  f1(i);
  ASSERT_EQ(43, i);
}
...
TEST(FixedFunction, classMethod1) {
  using namespace std::placeholders;
  A a;
  limb::tp::FixedFunction<int(const int &)> f(std::bind(&A::b, &a, _1));
  auto f1 = std::move(f);
  ASSERT_EQ(4, f1(4));
}
...
TEST(FixedFunction, classMethod2) {
  ...
  Foo<float> foo;
  foo.payload = 1.f;
  limb::tp::FixedFunction<int(int)> f(std::bind(&Foo<float>::bar<int>, &foo, _1));
  auto f1 = std::move(f);
  ASSERT_EQ(2, f1(1));
}
...
TEST(FixedFunction, lambda) {
  ...
  limb::tp::FixedFunction<std::string()> f([&s1]() { return s1; });
  auto f1 = std::move(f);
  ASSERT_EQ(s1, f1());
}
