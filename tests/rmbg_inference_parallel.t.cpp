...
constexpr int g_parrallelTasks = 4;
...
std::barrier sync_point(g_parrallelTasks);

void runTask(limb::AppBase *app, const limb::ImageTask &task, std::atomic<liret> &err) {
  sync_point.arrive_and_wait();
  liret ret = app->processImage(task);
  if (ret != liret::kOk) {
    err = ret;
  }
  return;
...
TEST_F(InferenceRunner, rmbg_parallel_verify) {
  const char pn[] = "RMBG-1.4";
  size_t pc = findProcessorByName(application.get(), pn);
  ASSERT_NE(pc, -1) << pn << " not found!";
  std::vector<std::thread> threads;

  std::atomic<liret> ret = liret::kOk;
  for (int i = 0; i < g_parrallelTasks; ++i) {
    std::ostringstream oss;
    oss << "./rmbg_parallel_test_output_" << i << ".png";
    limb::ImageTask task{.modelId = uint32_t(pc), .imageId = oss.str()};

    threads.emplace_back(runTask, application.get(), task, std::ref(ret));
  }
